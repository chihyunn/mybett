-- CreateTable
CREATE TABLE "Sport" (
    "id" TEXT NOT NULL PRIMARY KEY,
    "name" TEXT NOT NULL,
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- CreateTable
CREATE TABLE "Team" (
    "id" TEXT NOT NULL PRIMARY KEY,
    "name" TEXT NOT NULL,
    "sportId" TEXT NOT NULL,
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT "Team_sportId_fkey" FOREIGN KEY ("sportId") REFERENCES "Sport" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);

-- CreateTable
CREATE TABLE "BetType" (
    "id" TEXT NOT NULL PRIMARY KEY,
    "code" TEXT NOT NULL,
    "name" TEXT NOT NULL,
    "description" TEXT
);

-- CreateTable
CREATE TABLE "Bet" (
    "id" TEXT NOT NULL PRIMARY KEY,
    "sportId" TEXT NOT NULL,
    "teamAId" TEXT NOT NULL,
    "teamBId" TEXT NOT NULL,
    "selectedTeam" TEXT NOT NULL,
    "betTypeId" TEXT NOT NULL,
    "pAgent" REAL NOT NULL,
    "pMarket" REAL NOT NULL,
    "predictedEdge" REAL NOT NULL,
    "recommendedAmount" INTEGER,
    "actualAmount" INTEGER,
    "result" INTEGER,
    "realizedEdge" REAL,
    "profitLoss" REAL,
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "settledAt" DATETIME,
    CONSTRAINT "Bet_sportId_fkey" FOREIGN KEY ("sportId") REFERENCES "Sport" ("id") ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT "Bet_teamAId_fkey" FOREIGN KEY ("teamAId") REFERENCES "Team" ("id") ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT "Bet_teamBId_fkey" FOREIGN KEY ("teamBId") REFERENCES "Team" ("id") ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT "Bet_betTypeId_fkey" FOREIGN KEY ("betTypeId") REFERENCES "BetType" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);

-- CreateTable
CREATE TABLE "Balance" (
    "id" TEXT NOT NULL PRIMARY KEY DEFAULT 'main',
    "initialAmount" REAL NOT NULL DEFAULT 5000,
    "currentAmount" REAL NOT NULL DEFAULT 5000,
    "totalProfit" REAL NOT NULL DEFAULT 0,
    "totalBets" INTEGER NOT NULL DEFAULT 0,
    "wins" INTEGER NOT NULL DEFAULT 0,
    "losses" INTEGER NOT NULL DEFAULT 0,
    "updatedAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- CreateIndex
CREATE UNIQUE INDEX "Sport_name_key" ON "Sport"("name");

-- CreateIndex
CREATE UNIQUE INDEX "Team_sportId_name_key" ON "Team"("sportId", "name");

-- CreateIndex
CREATE UNIQUE INDEX "BetType_code_key" ON "BetType"("code");

-- CreateIndex
CREATE INDEX "Bet_sportId_idx" ON "Bet"("sportId");

-- CreateIndex
CREATE INDEX "Bet_teamAId_idx" ON "Bet"("teamAId");

-- CreateIndex
CREATE INDEX "Bet_teamBId_idx" ON "Bet"("teamBId");

-- CreateIndex
CREATE INDEX "Bet_betTypeId_idx" ON "Bet"("betTypeId");

-- CreateIndex
CREATE INDEX "Bet_createdAt_idx" ON "Bet"("createdAt");
